...
// Mijn grootste level is 40 (39 + 1 voor '\0') karakters breed, dus vandaar
// dat de MAX_GROOTTE 40 is.
const int MAX_GROOTTE = 40;

struct rooster_data {
  // Deze array van arrays bevat de karakters waaruit het doolhof bestaat
  char **grid;

  // Deze twee waardes bevatten de breedte en de hoogte van het doolhof.
  int breedte;
  int hoogte;

  // Deze variabele bevat de huidige toestand van het spel.
  toestand fase;
};
...
int rooster_bevat(const rooster *rp, int x, int y) {
  return x < rp->breedte && x >= 0 && y < rp->hoogte && y >= 0;
}
